 ...
 * based on <a href="http://logging.apache.org/log4j/1.2/">Log4J 1.2</a>, it is currently based on <a
 * href="http://www.slf4j.org/">SLF4J</a>.
 *
 * <P>To use this class, you first create a new instance by passing an
 * {@linkplain #ProgressLogger(Logger, long, TimeUnit, String) SLF4J logger, a time interval and a name for the items
 * that are being logged}. Information will be logged about the current state of affairs no more often than the given
 ...
 * display the given string. Then, each time you want to mark progress, you call usually {@link #update()} or
 * {@link #lightUpdate()}. The latter methods increase the item counter, and will log progress information if enough
 * time has passed since the last log (and if the counter is a multiple of {@link #LIGHT_UPDATE_MASK} + 1, in the case
 * of {@link #lightUpdate()}). Other update methods (e.g., {@link #update(long)}, {@link #updateAndDisplay()}, &hellip;)
 ...
 * information about the internal state of the logger (elapsed time, number of items per second) that can be printed or
 * otherwise processed. If {@link #update()} has never been called, you will just get the elapsed time. By calling
 * {@link #done()} instead of stop, this information will be logged for you.
 *
 * <P>Additionally:
 * <UL>
 ...
 * you can change its attributes and call {@link #start()} again to measure another activity.
 *
 * <P>A typical call sequence to a progress logger is as follows:
 * <PRE>
 * ProgressLogger pl = new ProgressLogger(logger, 1, TimeUnit.MINUTES); pl.start("Smashing pumpkins..."); ... activity
 * on pumpkins that calls update() on each pumpkin ... pl.done();
 * </PRE>
 *
 * <P>A more flexible behaviour can be obtained at the end of the
 * process by calling {@link #stop()}:
 * <PRE>
 * ProgressLogger pl = new ProgressLogger(logger, 1, TimeUnit.MINUTES, "pumpkins"); pl.start("Smashing pumpkins...");
 * ... activity on pumpkins that calls update() on each pumpkin ... pl.stop("Really done!");
 * pl.logger.info(pl.toString());
 * </PRE>
 *
 * <P>An instance of this class can also be used as a handy timer:
 * <PRE>
 * ProgressLogger pl = new ProgressLogger(); pl.start("Smashing pumpkins..."); ... activity on pumpkins (no update()
 * calls) ... pl.done(howManyPumpkins);
 * </PRE>
 *
...
public final class ProgressLogger {
    ...
    /**
     * Calls to {@link #lightUpdate()} will cause a call to {@link System#currentTimeMillis()} only if the current value
     * of {@link #count} is a multiple of this mask plus one.
     */
    ...
    /**
     * The time interval for a new log in milliseconds. Can be set at any time (e.g., using
     * {@link TimeUnit#toMillis(long)}).
     */
    ...
    public Object info;
    /**
     * The number of calls to {@link #update()} since the last {@link #start()} (but it be changed also with
     * {@link #update(long)} and {@link #set(long)}).
     */
    public long count;
    /**
    ...
    /**
     * Whether to display additionally the local speed, that is, the detected speed between two consecutive logs, as
     * opposed to the average speed since {@link #start()} (default: false).
     */
    public boolean displayLocalSpeed;
    /**
     * A fixed time unit for printing the speed. If {@code null}, {@link ProgressLogger} will choose a time unit that is
     * easy to understand. In some cases (e.g., machine-parsed logs) you might want to have always the speed in items
     * per {@linkplain TimeUnit#SECONDS seconds}, {@linkplain TimeUnit#MINUTES minues},
     * {@linkplain TimeUnit#HOURS hours} or {@linkplain TimeUnit#DAYS days}.
     */
     ...
     * an item in {@linkplain TimeUnit#NANOSECONDS nanoseconds}, {@linkplain TimeUnit#MICROSECONDS milliseconds},
     * {@linkplain TimeUnit#MILLISECONDS milliseconds}, {@linkplain TimeUnit#SECONDS seconds},
     ...
     * {@value #DEFAULT_LOG_INTERVAL} milliseconds to the {@linkplain Logger#ROOT_LOGGER_NAME root logger}.
     */
    ...
    /**
     * Creates a new progress logger logging every {@value #DEFAULT_LOG_INTERVAL} milliseconds to the
     * {@linkplain Logger#ROOT_LOGGER_NAME root logger}.
     *
     ...
     */
    public ProgressLogger(
            final Logger logger, final long logInterval, final TimeUnit timeUnit, final String itemsName
    ) {
        this.logger          = logger;
    ...
    /**
     * The <a href="http://www.slf4j.org/">SLF4J</a> logger used by this progress logger; it just returns the content of
     * the field {@link #logger}.
     */
    ...
    /**
     * Updates the internal count of this progress logger by adding one; if enough time has passed since the last log,
     * information will be logged.
     *
     * <p>This method is kept intentionally short (it delegates most of the work to an internal
     * private method) so to suggest inlining. However, it performs a call to {@link System#currentTimeMillis()} that
     * takes microseconds (not nanoseconds). If you plan on calling this method more than a few thousands times per
     * second, you should use {@link #lightUpdate()}.
     */
    public void update() {
        update(1);
    }
    ...
    /**
     * Updates the internal count of this progress logger by adding a specified value; if enough time has passed since
     * the last log, information will be logged.
     *
     * @see #update()
     */
    ...
    /**
     * Sets the internal count of this progress logger to a specified value; if enough time has passed since the last
     * log, information will be logged.
     *
     * @see #update()
     */
    ...
    private void updateInternal(final long currentTime) {
        ...
        if (logger().isInfoEnabled())
            logger().info(Util.format(count) + " " + itemsName + ", " +
                                  millis2hms(millis()) + ", " + itemsPerTimeInterval(
                    0, currentTime, startTime) + ", " + timePerItem(0, currentTime, startTime) +
                                  (displayLocalSpeed ? " [" + itemsPerTimeInterval(
                                          lastCount, currentTime, lastLogTime) + ", " + timePerItem(
                                          lastCount, currentTime, lastLogTime) + "]" : "") +
                                  (expectedUpdates > 0 ? "; " + Util.format(
                                          (100 * count) / expectedUpdates) + "% done, " +
                                          millis2hms(
                                                  millisToEnd) + " to end" : "") + freeMemory() + (info != null ? "; " + info : ""));
     ...
     * it will actually call {@link System#currentTimeMillis()} only if the new {@link #count} is a multiple of
     * {@link #LIGHT_UPDATE_MASK} + 1. This mechanism makes it possible to reduce the number of calls to
     * {@link System#currentTimeMillis()} significantly.
     *
    ...
    /**
     * Starts the progress logger, displaying a message, resetting the count and assuming that a given amount of time
     * has already passed.
     *
     * <p>The effect of the {@code alreadyElapsed} parameter is that the start time of this {@link ProgressLogger} will
     * be set to {@link System#currentTimeMillis()} minus {@code alreadyElapsed}.
     *
     * @param message        the message to display.
     * @param alreadyElapsed the number of milliseconds already elapsed.
     */
    ...
    /**
     * Completes a run of this progress logger and sets the internal counter, logging &ldquo;Completed.&rdquo; and the
     * logger itself.
     *
    ...
    /**
     * Returns the number of milliseconds between present time and the last call to {@link #start()}, if this progress
     * logger is running, or between the last call to {@link #stop()} and the last call to {@link #start()}, if this
     * progress logger is stopped.
     *
     * @return the number of milliseconds between present time and the last call to {@link #start()}, if this progress
     * logger is running, or between the last call to {@link #stop()} and the last call to {@link #start()}, if this
     * progress logger is stopped.
     */
    ...
    @Contract(pure = true)
    private @NotNull String millis2hms(final long t) {
     ...
     * are computed using the stop time. Otherwise, they are computed using the current time (i.e., the method call
     * time).
     *
    ...
    public String toString() {
        ...
        return "Elapsed: " + millis2hms(t) + (count != 0 ? " [" + Util.format(
                count) + " " + itemsName + ", " + itemsPerTimeInterval(0, stopTime, startTime) + ", " + timePerItem(
                0, stopTime, startTime) + "]" : "") + freeMemory();
    }
}
